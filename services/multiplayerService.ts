...
type OpponentUpdateCallback = (state: PlayerState) => void;
...
class MultiplayerService {
    ...
    public joinGlobalLobby(
        ...
        this.onGlobalPlayersUpdate = onUpdate; // Always update callback
    ...
    public joinLobby(lobbyId: string, player: { id: string, name: string }, onUpdate: (players: PlayerState[]) => void) {
        ...
        // Local cache of players to merge presence and broadcast data
        let currentPlayers: PlayerState[] = [];

        this.channel
            .on('broadcast', { event: 'player_update' }, (payload) => {
                // Handle live updates (score/distance)
                if (window.DEBUG_MODE) console.log('RX Broadcast:', payload);
            ...
            .on('presence', { event: 'sync' }, () => {
                ...
                if (window.DEBUG_MODE) console.log('Presence Sync:', newState);
                const presencePlayers: PlayerState[] = [];

                if (newState) {
                    Object.keys(newState).forEach(key => {
                        const p = newState[key][0] as any;
                        // Check if we have fresher data from broadcast
    ...
    public async broadcastState(state: PlayerState) {
        ...
        // Use Broadcast for high-frequency game state (Low Latency)
